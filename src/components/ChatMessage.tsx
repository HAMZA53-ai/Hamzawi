...
export const ChatMessageComponent: React.FC<{ message: ChatMessage; isLastMessage: boolean; loadingState: LoadingState; }> = ({ message, isLastMessage, loadingState }) => {
  ...
  const handleSpeak = (contentToSpeak: string) => {
    if (isSpeaking) {
      ...
      const html = marked.parse(contentToSpeak) as string;
    ...
    return (
        <div className="prose leading-relaxed">
            ...
            {contentParts.map((part, index) => {
                ...
                const parsedHtml = marked.parse(part.content, { gfm: true, breaks: true }) as string;
                return (
                    <div key={index} dangerouslySetInnerHTML={{ __html: parsedHtml }}/>
                );
            })}
